...
-- 저장 프로시저 정의 시작
CREATE PROCEDURE 프로시저이름( 매개변수 타입, 매개변수2 타입)
BEGIN
    -- 프로시저 내부에서 실행할 SWL 작성
...
-- 해당 사원의 최근 발령일자와 급DU를 반환하는 프로시저를 정의하시오.
-- OUT : 발령일자, 급여
...
CALL get_last_date_and_salary(20010, @last_date, @last_salary);

-- 결과 확인
SELECT @last_date AS '최근 발령 일자',
