...
-- 사원 테이블에서 입사 이랒로 부터 날짜, 연/월/일 을 추출하시오.
SELECT emp_no
        ,first_name
        ,DATE(hire_date) AS '입사 일자'
        ,YEAR(hire_date) AS '연'
        ,MONTH(hire_date) AS '월'
        ,DAY(hire_date) AS '일'
FROM employees;
...
-- 이번 달(4월)이 생일인 사원을 조회하시오.
SELECT *
FROM employees
WHERE MONTH(birth_date) = MONTH( NOW() );

-- 오늘로 부터 5년 뒤는 몇년도 인가요?
...
SELECT TIMESTAMPDIFF( YEAR, from_date, to_date) '부서 근속연수'
FROM dept_emp
