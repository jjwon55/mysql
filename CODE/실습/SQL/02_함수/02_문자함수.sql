...
-- 문자열을 대문자, 소문자, 첫글자만 대문자로 변환하는 쿼리를 작성하시오.
-- 원문 : 'AlohA WorlD~!'
...
-- 문자열의 글자 수를 반환하는 쿼리르 ㄹ작성하시오.
-- 영문, 공백 : 1Byte
-- 한글       : 3Byte
SELECT LENGTH('ALOHA CLASS') AS '바이트 수'
      ,LENGTH('알로하 클래스') AS '바이트 수'
      ,CHARACTER_LENGTH('ALOHA CLASS') AS '글자수'
      ,CHARACTER_LENGTH('알로하 클래스') AS '글자수'
FROM dual;
...
FROM employees;
 ...
 -- LOCATE('찾을 문자', 'wwwaloha@kakao.com')
